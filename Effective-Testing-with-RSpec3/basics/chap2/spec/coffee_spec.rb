...
=begin
  ...
  $ rspec --only-failures
    #=> run the rspec normally the first time to find and record all the failures
    ...
    #=> you need to define a path where Rspec can save the resulting data in your 'spec_helper.rb' file
    #=> add the following:
  ...
  and configuring your spec_helper file

    RSpec.configure ​do​ |config|
       config.filter_run_when_matching(​focus: ​​true​)​
    end

  When you run rspec(no options required) only that example/group are executed
  ...
  pending examples:
    - use the 'pending' keyword, with an explanation
